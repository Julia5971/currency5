...
**⚠️ 중요 업데이트**: 한국은행 Open API가 8월 말까지 사용 불가로 인해 **ExchangeRate-API** 등 대체 API 사용
...
- **Day 1-2**: Chart.js 환율 차트, 과거 데이터 시각화
 x 축: 날자. y축 원화대비 환율. 
 각 외환마다 하나의 차트로 종류별로 생성
 차트는 정중앙에 4개가 보여야 하고 크기는 1/6 화면에 2*2로 표현
 그래프 색은, kor 은 파랑, jap는 노랑, eur은 초록 cny는 빨강으로 표시해줘
